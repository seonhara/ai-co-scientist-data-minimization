...
def run_experiments(X_train, X_test, y_train, y_test, strategies=("H", "P", "C", "R"), levels=4):
    ...
    for strategy in strategies:
        for level in range(levels):
            ...
            if X_train_lvl.shape[1] == 0:
                raise ValueError(f"[EMPTY FEATURES] strategy={strategy}, level={level}. "
                                 f"Check STRATEGY_GROUPS prefixes.")
...
def make_delta_table(results_df: pd.DataFrame, metric="F1"):
    base = results_df[results_df["Level"] == 0][["Strategy", "Model", metric]].rename(
        columns={metric: f"{metric}_L0"}
    )
...
def main():
    ...
    delta_df.to_csv("results_delta_f1_from_level0.csv", index=False)
    print("[Saved] results_delta_f1_from_level0.csv")
